...
class MultiStreamPreprocessor:
    ...
    def __init__(self, sample_rate: int = 44100, cache_dir: str = None, use_fast_hpss: bool = True):
    ...
    def process(self, audio_stereo: torch.Tensor, cache_key: str = None) -> dict:
        ...
        if self.use_fast_hpss:
            harmonic, percussive = self._fast_hpss(mid)
        else:
    ...
    @staticmethod
    def _fast_hpss(y: torch.Tensor, kernel_size=1024):
        # y: (1, T)
        T = y.shape[-1]
        harmonic = torch.nn.functional.avg_pool1d(
            y.unsqueeze(0), kernel_size, stride=1, padding=0
        ).squeeze(0)

        harmonic = torch.nn.functional.pad(harmonic, (kernel_size // 2, T - harmonic.shape[-1] - kernel_size // 2))
